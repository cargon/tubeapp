	    ...
	    $stmt->bind_param('d', $vid);
           $filename = $video->getFilename();
            ...
            if($stmt->execute())
            {
                ...
                if($stmt->execute())
                {
                	$stmt = $this->dblink->prepare("DELETE FROM favoritos WHERE vid=?");
                	$stmt->bind_param('d', $vid);
                	if($stmt->execute())
                	{
...
//______________________________________________________________________________________________        
        function getMostRated($inicio,$qtd)
        {
        	if(empty($this->uid))
        	{
            	$stmt=$this->dblink->prepare("SELECT vid, IF(votos=0,0,rating/votos) FROM ratings ORDER BY IF(votos=0,0,rating/votos) DESC LIMIT ?,?");
            	$stmt->bind_param('dd',$inicio,$qtd);
            $stmt->execute();
            }
			else
